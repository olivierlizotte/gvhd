...
Node peptidome = graphDb.getNodeById(875149);
	...
	HashMap<Node, ArrayList<Relationship>> proteins = new HashMap<Node, ArrayList<Relationship>>();
...
//			   (Double)peptide.getProperty("Highest Score") >= 23.175 &&
						...
						for (Relationship relSequence : sequence.getRelationships())
						{
							...
							if (NodeHelper.getType(relSequence.getOtherNode(sequence)).equals("Protein Sequence"))
							{
								...
								if(!proteins.containsKey(protein))
									proteins.put(protein, new ArrayList<Relationship>());
								proteins.get(protein).add(relSequence);
								found++; 
							}
						}
						if(found == 0)
	...
	for(Relationship relProtein : proteins.keySet())
	{
		...
		if(protein.hasProperty("Chromosome number"))//Else, might be EBV Database
		{
			String proteinID = protein.getProperty("Protein id").toString();
			...
			for(Node peptide : proteins.get(protein))
			{
				...
				if (!proteinID.startsWith("Ref") && (proteinID.startsWith("M") || proteinID.startsWith("R")))
