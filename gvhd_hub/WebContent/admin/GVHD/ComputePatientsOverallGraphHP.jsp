	...
	for (Relationship idRel : project.getRelationships())
	{
		...
		if (NodeHelper.getType(patient).equals("Patient"))
		{
				...
				if (NodeHelper.getType(sample).equals("Sample"))
				{
					if(sample.hasProperty("Sample"))
					{
						...
						if(sample.getProperty("Sample").toString().endsWith("A") &&
								sample.hasProperty("Ratio HP Libre") &&
								sample.hasProperty("Ratio HP Total"))
						{
							desLibreDivider = NodeHelper.PropertyToDouble(sample.getProperty("Ratio HP Libre"));
							desTotalDivider = NodeHelper.PropertyToDouble(sample.getProperty("Ratio HP Total"));
						}//*/
						if(sample.getProperty("Sample").toString().endsWith("A"))
						{
			...
			if(dateY > 0 && dateT0 > 0 && desLibreDivider > 0)
			{
				...
				for (Relationship sampleRel : patient.getRelationships())
				{
					...
					{				
						...
						if(daySince <= 0)
						{
							...
							if(sample.hasProperty("Ratio HP Libre") && sample.hasProperty("Ratio HP Total"))
							{
								double tmp = NodeHelper.PropertyToDouble(sample.getProperty("Ratio HP Libre"));
								double tmp2 = NodeHelper.PropertyToDouble(sample.getProperty("Ratio HP Total"));
				...
				json += ",{values:[" + arrayDESLibre.substring(1) + "], key: 'HP Patient " + patient.getProperty("Name") + "'}";
			}
		}
	}
