...
try{
	...
	File file = new File("G:\\Thibault\\Olivier\\Projet GVHD\\gvhd\\ControlCD_Transpose.csv");	 
	...
	while((line = bufRdr.readLine()) != null)
	{
		...
		if(cols.length > 1)
		{
			Node control = Helper.CreateControlCD(graphDb, project, "CD");
			if(NodeHelper.isNumeric(cols[0]))
				control.setProperty("Control", Double.parseDouble(cols[0]));
			else
				control.setProperty("Control", cols[0]);
			...
			for(int i = 1; i < cols.length && i < properties.length; i++)
			{
				if(!cols[i].isEmpty())
				{
					Node toUpdate = control;
					if(properties[i].startsWith("Total"))
					{
						if(total == null)
						{
							total = graphDb.createNode();
							control.createRelationshipTo(total, newNodeRel);
					...
					else if(properties[i].startsWith("Libre"))
					{
						if(libre == null)
						{
							...
							control.createRelationshipTo(libre, newNodeRel);
