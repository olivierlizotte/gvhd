				...
				if (NodeHelper.getType(sample).equals("Sample"))
				{
					if(sample.hasProperty("Sample"))
					{
						...
						if(sample.getProperty("Sample").toString().endsWith("A") &&
								sample.hasProperty("Ratio LP Libre") &&
								sample.hasProperty("Ratio LP Total"))
						{
							desLibreDivider = NodeHelper.PropertyToDouble(sample.getProperty("Ratio LP Libre"));
							desTotalDivider = NodeHelper.PropertyToDouble(sample.getProperty("Ratio LP Total"));
						}//*/
						if(sample.getProperty("Sample").toString().endsWith("A"))
						{
			...
			if(dateY > 0 && dateT0 > 0 && desLibreDivider > 0)
			{
				...
				for (Relationship sampleRel : patient.getRelationships())
				{
					...
					{				
						...
						double daySince = ((formater.parse(date).getTime() - dateT0) /(1000.0*60*60*24)) - dateY;
						if(daySince <= 0)
						{
							//if(daySince >= 0 && daySince <= dateY && 
							if(sample.hasProperty("Ratio LP Libre") && sample.hasProperty("Ratio LP Total"))
							{
								double tmp = NodeHelper.PropertyToDouble(sample.getProperty("Ratio LP Libre"));
								double tmp2 = NodeHelper.PropertyToDouble(sample.getProperty("Ratio LP Total"));
								des.add(new Pair(daySince, new Pair(tmp / desLibreDivider, tmp2 / desTotalDivider)));
							}
						}
					}
				}
				...
				json += ",{values:[" + arrayDESLibre.substring(1) + "], key: 'LP Patient " + patient.getProperty("Name") + "'}";
			}
		}
	}
